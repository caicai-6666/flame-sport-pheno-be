...
- **赛季参与**：按未开始、进行中、结算中和已结束管理赛季生命周期，查询当前赛季并检查用户参与状态和报名时间；赛季开始配置保护期内暂停项目锁定、凭证上传和礼品兑换等客户业务写入。
- **运动项目**：查询项目与挑战规则，锁定赛季项目及挑战等级，并查询已锁定项目的完成进度。
- **运动凭证**：读取项目上传配置，上传 JPG 凭证，查询当前赛季、结算中和已结束赛季凭证，并查询和补交结算中赛季开放的指定记录；可由每日 DeepSeek 文本初审写入审核意见、初审状态和项目进度。
- **工作通知**：定时发送初审、终审和奖品发放产生的钉钉 Markdown 工作通知，并同步送达状态和重试失败任务。
- **排行榜**：定时统计当前赛季初审通过的有效凭证数量并生成排行榜快照；减重挑战月初不计数，月末同项目最多计一次。
- **积分商城**：查询商品和积分流水，使用积分兑换商品。
- **本地资源**：管理头像、项目图标、商品图片、运动凭证图片和可由管理端覆盖的 WebP 活动海报。
...
应用启动时会创建本地资源目录，并启动认证缓存清理、排行榜快照刷新、生产模式钉钉工作通知，以及可选的 DeepSeek 文本初审任务。
