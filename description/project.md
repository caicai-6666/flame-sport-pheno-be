# 项目概况

本文档负责说明 `flame-sport-pheno-be` 的业务流程、领域对象、代码分层和当前阶段限制。完整文档导航参见[项目文档地图](README.md)，文档新增与修改规范参见[项目文档撰写规范](document-style.md)。

## 项目定位

本项目是企业运动赛季平台的后端服务。用户可以在活动赛季中选择运动项目和挑战等级、上传运动凭证、查看排行榜，并在赛季结算后使用积分兑换商品。

项目当前处于开发和联调阶段，已经打通主要用户侧流程，部分后台管理和生产化能力尚未实现。

---

## 核心业务流程

```text
用户使用 auth_code 登录
  -> 获取当前激活赛季
  -> 锁定赛季要求数量的运动项目
  -> 锁定统一的挑战等级
  -> 正式参与当前赛季
  -> 按已锁定项目上传运动凭证
  -> 后台任务生成排行榜快照
  -> 赛季进入结算中，完成审核和积分结算
  -> 赛季标记已结束
  -> 用户使用全局积分兑换商品
```

当前系统将“存在 `season_user` 且已经设置 `level_id`”视为用户正式参与赛季。只选择项目但没有选择挑战等级时，用户尚未完成参与流程。

---

## 核心领域对象

| 对象 | 作用 |
| --- | --- |
| `department` | 企业部门主数据，用于关联用户和展示排行榜部门信息 |
| `user` | 用户基础资料，不保存登录凭据和积分余额 |
| `season` | 赛季周期、未开始至已结束的生命周期状态，以及必须选择的项目数量 |
| `project` | 平台支持的运动项目 |
| `project_level` | 可选择的挑战等级及奖励积分 |
| `project_rule` | 某个运动项目在不同等级下的挑战规则 |
| `project_upload_config` | 项目允许使用的凭证类型和上传提示 |
| `season_user` | 用户在一个赛季中的参与进度、等级和最终积分 |
| `season_user_project` | 用户在当前赛季锁定的运动项目 |
| `proof_record` | 用户上传的运动凭证及审核状态 |
| `season_supplement_eligibility` | 结算中赛季按原凭证开放的用户补传资格 |
| `leaderboard_snapshot` | 当前赛季有效凭证数量的排行榜快照 |
| `point_record` | 用户跨赛季累计的积分变动流水 |
| `product` | 积分商城中的可兑换商品 |

`season_user` 是赛季业务的核心关联对象。它把用户和赛季连接起来，项目选择、凭证、排行榜和赛季结算都围绕它展开。

---

## 当前已实现能力

- 通过钉钉企业内部 H5 微应用 `auth_code` 登录、认证缓存和受保护接口依赖注入。
- 生产模式首次登录时同步用户与部门基础资料，开发模式使用本地已有用户登录。
- 当前赛季查询、参与状态和报名期限检查。
- 运动项目、挑战规则、项目锁定、挑战等级锁定和已锁定项目完成进度查询；赛季开始配置保护期内会暂停客户业务写入，同时保留建议提交。
- 项目上传配置、凭证图片上传并统一保存为 WebP、可选图片分段定位校验与保存、赛季内补传、按运动日期的一日一凭证和历史记录查询；定时初审按数量阈值或第三次不足阈值的非空扫描触发，按用户项目分组并发，只处理进行中赛季并在结束前 5 分钟停止扫描；完整图片和规则已传入工作流，支持按定位在内存中生成记录图片，校验填写的运动日期属于原赛季，并渲染包含项目规则、赛季起止日期、编号图片和备注的消息，模型节点直接使用 OpenAI 异步 Responses API 发送图文消息并校验严格五字段输出，管理端可立即初审当前或历史赛季的非补交遗留记录，结算期补交记录使用资格创建时固化的规则专门初审；初审失败时同步创建待发送通知，重传会先撤销旧版本进度再按新版本重算。
- 当前用户查询结算中赛季仍开放补传资格的原凭证信息，并按原凭证 ID 补交图片与备注；补传截止时间和次数限制尚未实现。
- 生产模式定时发送初审、终审和奖品发放产生的钉钉 Markdown 工作通知；已读和未读统一记为送达，明确失败的通知会重新发送。
- 当前赛季初审通过凭证的排行榜快照定时刷新和查询；阶段型项目的月初记录不计数，月末通过按项目只计一次。
- 商品查询、积分流水查询和积分兑换。
- 登录用户提交建议或反馈。
- 头像、项目图标、商品图片、凭证图片和固定活动海报的本地文件管理；管理端可在 Docker 内部读取并覆盖 WebP 海报。

---

## 当前阶段限制

- 钉钉企业内部应用凭证与 access token 使用进程内缓存，多实例部署时不会共享 token；认证和部分业务配置也不会共享状态。
- 钉钉返回的 `userId` 直接作为本地 `user.id`；首次登录会同步姓名、头像和一个所属部门，但后续不会自动持续同步人员资料。
- 尚未实现审核人员后台、赛季结束统一审核和积分结算流程。
- 商品兑换暂不处理库存、订单和独立兑换记录。
- 图片保存在本地文件系统，尚未接入对象存储。
- 当前仓库尚未建立完整的自动化测试和数据库迁移体系。

---

## 代码编排

项目采用 FastAPI 模块化单体结构，主要调用方向为：

```text
router -> service -> repository -> model
```

| 目录 | 职责 |
| --- | --- |
| `app/routers/` | HTTP 路径、参数解析、鉴权和数据库会话依赖注入 |
| `app/services/` | 业务校验、流程编排、响应组装和事务边界 |
| `app/agent/` | `preliminary_review` 包中的模型调用、提示词、类型、节点和工作流；业务编排和定时调度仍由 Service 与 `core` 承担 |
| `app/repositories/` | SQL 查询、关联查询、行锁、写入和 `flush` |
| `app/models/` | SQLModel 数据表映射 |
| `app/core/` | 配置、数据库、认证、缓存、运行时状态、文件存储和后台任务 |
| `app/main.py` | FastAPI 应用创建、路由注册和应用生命周期 |

实现时应保持以下边界：

- `Router` 保持轻量，不承载跨表业务规则。
- `Service` 负责业务异常、事务提交与回滚，以及多个 `Repository` 的协作。
- `Repository` 聚焦数据访问，不处理 HTTP 请求和响应。
- `Model` 描述数据库结构，不承担业务流程。
- 通用基础设施放入 `app/core/`，不要复制到各业务模块。

---

## 文档入口

文档分类、业务主题映射和推荐阅读顺序统一维护在[项目文档地图](README.md)。本文档不重复维护具体文件清单，避免导航信息分散后失效。

新增或修改 Markdown 文档前，应阅读[项目文档撰写规范](document-style.md)；功能变更需要同步哪些文档，以[文档维护说明](dev/documentation.md)为准。

---

## 开发与文档维护规则

- 新增或修改接口后，同步更新对应的 `api/` 和 `business/` 文档。
- `db/` 是数据库设计依据。除非用户明确要求调整表结构，否则不要在接口开发中顺带修改数据库文档或模型。
- 如果实现、API 文档和业务文档存在冲突，先明确期望行为，不要自行扩大修改范围。
- 保留当前项目的分层方式和异步调用风格，优先复用已有 Service、Repository 和 `core` 能力。
- 当前阶段允许有明确记录的简化实现，不要在无需求支撑时引入大型框架或无关重构。

本地运行、MySQL、资源文件和文档维护细节分别参见：

- [`local_run.md`](dev/local_run.md)
- [`mysql_docker.md`](dev/mysql_docker.md)
- [`docker_compose.md`](dev/docker_compose.md)
- [`assets.md`](dev/assets.md)
- [`mock_data.md`](dev/mock_data.md)
- [`documentation.md`](dev/documentation.md)
