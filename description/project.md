...
- 通过 `auth_code` 登录、认证缓存和受保护接口依赖注入。
- 用户资料完整度检查和身高更新。
- 当前赛季查询、参与状态和报名期限检查。
- 运动项目、挑战规则、项目锁定和挑战等级锁定。
- 项目上传配置、JPG 凭证上传、当天重复上传和历史记录查询。
- 当前赛季排行榜快照的定时刷新和查询。
- 商品查询、积分流水查询和积分兑换。
...
- 联调阶段暂时将 `auth_code` 直接作为 `user_id`，尚未接入真实外部认证系统。
- 认证、当前赛季和部分业务配置使用进程内缓存，多实例部署时不会共享状态。
