...
首次增加礼品状态字段的旧环境可参考以下脚本：

```text
script/migrate-point-record-gift-distribution-status.sql
```

该脚本只覆盖 `pending` 和 `distributed` 两态，不等同于本页当前定义的三态完整结构。生产执行前必须根据目标数据库的实际结构制定升级 SQL，并核对历史兑换记录；不得自动推断历史礼品是否已经发放或被拒绝，避免重复发放或错误拒绝。

> **警告**
>
> MySQL DDL 会隐式提交。执行结构迁移前必须完成数据库备份、停止礼品审核及积分写入并预留维护窗口。本仓库不会自动执行生产迁移。
