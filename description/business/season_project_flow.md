# 赛季参与和项目锁定流程

## 核心对象

| 表 | 作用 |
| --- | --- |
| `season` | 赛季配置和当前激活状态 |
| `season_user` | 用户在赛季中的参与记录 |
| `season_user_project` | 用户锁定的赛季项目 |
| `project_level` | 挑战等级和奖励积分 |

赛季状态按以下生命周期流转：

| `season.status` | 含义 | 客户端行为 |
| --- | --- | --- |
| `0` | 未开始 | 不可参与、上传，不展示赛季凭证 |
| `1` | 进行中 | 作为当前赛季，允许参与、上传和查看排行榜 |
| `2` | 结算中 | 停止参与和上传，未定稿凭证不进入客户端历史 |
| `3` | 已结束 | 赛季凭证进入客户端历史 |

正常流转为 `0 -> 1 -> 2 -> 3`。结算中和已结束均不属于当前激活赛季。

---

## 当前赛季

当前激活赛季由以下条件确定：

```text
season.status = 1
ORDER BY season.start_date DESC
LIMIT 1
```

当前赛季不使用进程内缓存。赛季查询、参与检查、项目锁定、项目进度、凭证和排行榜等赛季相关入口均直接查询数据库，确保后台修改 `season.status` 后立即生效。

当没有 `status = 1` 的赛季时，当前赛季查询会返回 `404`。排行榜启动刷新和定时刷新则会跳过本次执行并记录普通信息日志；没有激活赛季（包括只有结算中赛季）是冷启动和赛季切换空窗期的正常状态，不会导致应用启动失败。

---

## 项目锁定

用户锁定项目时：

1. 校验请求赛季必须是当前激活赛季。
2. 校验项目存在且 `project.status = 1`。
3. 查询或创建 `season_user`。
4. 查询 `season_user_project` 是否已有记录。
5. 如果已有效锁定，直接返回成功。
6. 如果锁定数量超过 `season.required_project_count`，返回冲突。
7. 写入或重新激活 `season_user_project`；新锁定项目的 `completion_progress` 初始化为 `0.0000`。
8. 将 `season_user.status` 更新为有效锁定项目数量。

查询已锁定项目和完成进度时同样要求请求中的 `season_id` 是数据库当前激活赛季；没有激活赛季或传入旧赛季 ID 时，不返回该赛季的客户端数据。

---

## 等级锁定

用户锁定挑战等级前，必须满足：

```text
season_user.status == 当前赛季 required_project_count
season_user.level_id IS NULL
project_level.status = 1
```

等级锁定成功后：

```text
season_user.level_id = project_rule_level_id
season_user.participated_at = 当前时间
```

`participated_at` 仅在首次成功锁定等级时写入，代表用户正式报名时间；只锁定项目时保持为空。

---

## 正式参与判断

当前系统将以下条件视为用户正式参与赛季：

```text
season_user 存在
season_user.level_id IS NOT NULL
```

这意味着用户只锁定项目但没有锁定挑战等级时，还不算正式参与。

---

## 报名时间与抢先参与

`SEASON_PARTICIPATION_ALLOWED_DAYS` 控制包含赛季开始日在内的 `N` 个自然日报名窗口，统一按 `Asia/Shanghai` 计算。当今天与开始日的日期差 `>= N` 时，报名截止，默认 `N = 7`。开始日前已被后台激活的赛季仍允许抢先参与。

例如赛季于 2026 年 9 月 1 日开始，配置为 `7` 时，报名截止时间为 9 月 8 日 00:00，不包含截止时刻。配置保护期占用报名窗口，不顺延截止时间；保护期为 `24` 小时时，开始日后的实际可报名区间为 9 月 2 日 00:00 至 9 月 8 日 00:00。

参与检查对尚未正式报名的用户执行截止校验；项目锁定和等级锁定两个写接口也独立执行同一校验，截止后返回 `403`，不创建或更新报名记录。已选项目但未锁定等级的用户不能在截止后完成报名。已正式报名的用户仍可查询参与状态并正常使用赛季内业务，报名期限不限制其后续凭证上传。

---

## 赛季开始配置保护期

`ACTIVE_SEASON_CONFIG_EDIT_WINDOW_HOURS` 同时定义管理端配置调整窗口和客户业务写入保护期。客户后端固定按 `Asia/Shanghai` 计算以下区间：

```text
season.start_date 当日 00:00
<= 当前时间
< season.start_date 当日 00:00 + 配置小时数
```

保护期内拒绝项目锁定、挑战等级锁定、凭证上传、礼品兑换，以及首次钉钉登录产生的本地用户初始化。已经存在的用户仍可登录和使用查询能力；建议提交不受保护期影响。管理端内部资源写入、初审、排行榜刷新和通知发送等系统任务也不属于客户业务写入。

当前赛季接口会返回上海时区的服务端时间、保护期起止时刻和当前冻结状态，供客户端展示“预计若干小时后开放”并提前禁用写入口。项目内容和其他查询能力在保护期内保持可用；接口提示只改善用户体验，不能替代后端强校验。

赛季提前激活但尚未到开始日时不冻结，继续允许抢先参与；到达截止时刻后立即恢复写入。配置为 `0` 时保护期为空，没有激活赛季时不限制上述写入。若数据库同时存在多个激活赛季，客户业务写入失败关闭并返回冲突，避免任意选择一个赛季计算窗口。

保护期校验在 Service 的业务事务内共享锁定激活赛季，并且必须早于数据库或文件写入。凭证上传还应在创建目录、读取和转换图片前完成校验，避免被拒绝的请求产生本地副作用。

---

## 项目完成进度

`season_user_project.completion_progress` 表示用户锁定项目在本赛季的完成比例，范围为 `0`～`1`。该字段不参与项目锁定数量和正式参与判断。

后续定时初审任务会筛查超过最小等待时间仍待审的凭证。初审通过时，后端在同一事务中累加对应项目进度并封顶到 `1`；初审失败不改变进度。赛后终审只用于最终结算，不回溯改变赛季内进度。
