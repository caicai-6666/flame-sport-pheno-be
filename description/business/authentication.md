...
3. 若钉钉返回头像地址，下载 JPEG、PNG 或 WebP 图片并统一转换、压缩为不超过 300 KiB 的 JPEG 后保存到本地 `assets/images/avatar/`；文件名使用安全化后的 `userId` 加小写 `.jpg`，例如 `james.jpg`，`user.avatar_url` 保存为 `/james.jpg`。
...
钉钉头像下载仅接受 HTTP(S) 地址、JPEG/PNG/WebP 格式和 5MB 以内的文件。保存前会统一转换为真实 JPEG 字节，并优先降低 JPEG 质量、必要时缩小尺寸，使最终文件不超过 300 KiB；所有本地头像后缀均为小写 `.jpg`。钉钉没有头像时允许用户以空头像完成初始化；头像地址存在但无法下载、无法转换、压缩后仍无法满足大小限制或格式不合规时，初始化失败且不会创建用户。
