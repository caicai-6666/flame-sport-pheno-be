...
当前时间处于赛季开始配置保护期时，接口返回 `409` 且不创建 `season_user` 或项目锁定记录。保护期按 `Asia/Shanghai` 的 `season.start_date 00:00` 起算。

报名截止后返回 `403`，不创建或更新赛季用户及项目锁定记录，包括已有锁定的重复请求。截止时间采用[报名时间与抢先参与](../business/season_project_flow.md#报名时间与抢先参与)中的上海时区口径。
...
当前时间处于赛季开始配置保护期时，接口返回 `409` 且不修改挑战等级或报名时间。

报名截止后返回 `403`，即使用户已在截止前锁满项目，也不能再锁定等级完成报名。该校验由写接口独立执行，不依赖客户端预先调用参与检查接口。

每次请求都会直接查询数据库激活赛季；没有激活赛季时返回 `404`，请求中的 `season_id` 不是当前激活赛季时返回 `400`。

两个写接口的报名期限和保护期错误如下；保护期校验先于报名期限校验：

| 状态码 | 场景 | detail |
| --- | --- | --- |
| `403` | 已到达报名截止时间 | `已超过赛季报名时间` |
