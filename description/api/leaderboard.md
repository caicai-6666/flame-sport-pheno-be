...
`leaderboard_snapshot` 由后端定时任务刷新，刷新间隔由 `LEADERBOARD_REFRESH_INTERVAL_SECONDS` 控制。默认启动时刷新一次，之后每 86400 秒刷新一次；每日文本初审任务写入结果后也会立即刷新。每次刷新会统计赛季开始后、本次刷新时刻前已初审通过的有效凭证；待初审和初审失败凭证不计入，赛后终审不回溯改变排行榜统计。减重挑战的月初记录不计入，月末记录按同一用户同一项目最多一次统计。
