...
| 方法 | 路径 | 是否鉴权 | 说明 |
| --- | --- | ---: | --- |
| GET | `/shop/product_info` | 是 | 获取商城可见商品列表 |
| GET | `/shop/point_flow` | 是 | 获取当前用户积分流水 |
| POST | `/shop/consume` | 是 | 兑换商品并写入积分流水 |
...
```http
GET /image/product?filename={encodeURIComponent(image_url)}
```
